...
test("update text output reports already up-to-date without invoking npm", async (t) => {
  ...
  assert.match(harness.stdout.read(), /Current version: 1\.0\.2/);
  assert.match(harness.stdout.read(), /Latest version: 1\.0\.2/);
...
test("passive update notifications still run for ordinary commands", async (t) => {
  ...
  assert.match(stderr.read(), /Update available: Cogcoin 1\.0\.2 -> 1\.0\.3/);
});
