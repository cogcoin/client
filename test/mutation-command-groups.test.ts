...
test("mutation helpers classify canonical commands without bleeding into neighbors", () => {
  ...
  assert.equal(isRegisterMutationCommand("anchor"), false);
  ...
  assert.equal(isSellOrUnsellMutationCommand("sell"), true);
  assert.equal(isSellOrUnsellMutationCommand("unsell"), true);
  assert.equal(isSellOrUnsellMutationCommand("buy"), false);
